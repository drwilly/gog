...
SRC_URI="
	${GOG_PN}_${GOG_PV}_26846.sh
	dlc_horizonsignal? ( ${GOG_PN}_horizon_signal_${GOG_PV}_26846.sh )
	dlc_anniversaryportraits? ( ${GOG_PN}_anniversary_portraits_${GOG_PV}_26846.sh )
	dlc_utopia? ( ${GOG_PN}_utopia_${GOG_PV}_26846.sh )
"
...
src_prepare() {
	find "${S}" \
		...
		-type f -exec chmod -x {} + \
		|| die
	chmod +x *.so.* *.so stellaris *.py || die

	default
}

src_install() {
	make_wrapper "${PN}" ./stellaris "/opt/gog/${PN}"
